...
import { Search, Loader2, TrendingUp, TrendingDown, DollarSign, Calculator } from 'lucide-react';
import { useUser } from '../../contexts/UserContext';
import { useAuth } from '../../contexts/AuthContext';
import { getStockPrice, buyStockEnhanced, sellStockEnhanced, StockData } from '../../lib/enhancedMarketstack';
import { searchStocks } from '../../lib/ninjaApi';
import { validateTransaction } from '../../lib/portfolioTracking';
import { cn } from '../../lib/utils';

const EnhancedTradeForm: React.FC = () => {
  const { user } = useAuth();
  ...
  const handleTrade = async () => {
    ...
    try {
      if (mode === 'buy') {
        await buyStockEnhanced(user.id, stockData.symbol, quantity);
      } else {
        await sellStockEnhanced(user.id, stockData.symbol, quantity);
      }
  ...
  return (
    <div className="card p-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg font-semibold">Enhanced Trading</h2>
